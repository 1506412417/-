...
class Rbac {
    ...
    static public function getAccessList($authId) {
        $access = M()
            ->field('ru.user_id, na.id, na.name action_name, nc.name controller_name, nm.name module_name')
            ->table(C('RBAC_USER_TABLE'))
            ->alias('ru')
            ->join('join ' . C('RBAC_ACCESS_TABLE') . ' a on ru.role_id=a.role_id')
            ->join('left join ' . C('RBAC_NODE_TABLE') . ' na on na.id=a.node_id')
            ->join('left join ' . C('RBAC_NODE_TABLE') . ' nc on nc.id=na.pid')
            ->join('left join ' . C('RBAC_NODE_TABLE') . ' nm on nm.id=nc.pid')
            ->where(['ru.user_id'=>$authId, ])
            ->select();
        $accessList = [];
        foreach($access as $a) {
            //转换大写
            $mName = strtoupper($a['module_name']);
            $cName = strtoupper($a['controller_name']);
            $aName = strtoupper($a['action_name']);
            $accessList[$mName][$cName][$aName] = $a['id'];
        }
        return $accessList;
/*        // Db方式权限数据
