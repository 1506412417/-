...
class RoleController extends Controller
{
	...
	public function editAction(){
		...
		if(IS_POST){
			if($model->create()){//校验
				...
				session('data', $_POST);
				$this->redirect('edit', ['id'=>I('post.id')]); // 重定向到添加
			} 
		}else{
			$this->assign('message',session('message'));
			session('message',null);
			// 获取当前编辑的内容, 如果是编辑错误,则显示错误的内容, 如果是没有错误, 则显示原始数据内容
			 $this->assign('data', is_null(session('data')) ? $model->find(I('get.id')) : session('data'));
			 session('data',null);
	...
	public function multiAction(){
		...
		switch ($operate) {
			case 'delete':
				$model=M('Role');
				$model->where(['id'=>['in',I('post.selected')]])->delete();
				break;
		}
	...
	{
		if (IS_POST) {
			$role_id = I('post.id');//角色id
			$modelNode = M('Node');
			//获取到的节点id进行处理
			$rows = array_map(function($node_id) use($role_id, $modelNode   ) {
				// 利用节点id获取level, 再获取对应的module
				$node = $modelNode // action级别
				->alias('a')
					->field('a.level, m.name')
					->join('left join __NODE__  c on c.id=a.pid')//控制器级别
					->join('left join __NODE__  m on m.id=c.pid')//模块级别
					->where(['a.id'=>$node_id])
					->find();
				return [
					'role_id'   => $role_id,
			...
			M('Access')->where(['role_id'=>$role_id])->delete();//删除原有权限
			M('Access')->addAll($rows);//添加新权限
			$this->redirect('access', ['id'=>$role_id]);
		} else {
			// 显示全部的node
			$nodeNested = D('Node')->getNested();
			$this->assign('nodeNested', $nodeNested);
			// 勾选当前角色拥有的node
			$this->assign('accessList', M('Access')->where(['role_id'=>I('get.id')])->getField('node_id', true));
