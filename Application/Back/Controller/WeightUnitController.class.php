...
class WeightUnitController extends Controller
{
	...
	public function editAction(){
		...
		if(IS_POST){
			if($model->create()){//校验
				...
				session('data', $_POST);
				$this->redirect('edit', ['weight_unit_id'=>I('post.weight_unit_id')]); // 重定向到添加
			} 
		}else{
			$this->assign('message',session('message'));
			session('message',null);
			// 获取当前编辑的内容, 如果是编辑错误,则显示错误的内容, 如果是没有错误, 则显示原始数据内容
			 $this->assign('data', is_null(session('data')) ? $model->find(I('get.weight_unit_id')) : session('data'));
			 session('data',null);
	...
	public function multiAction(){
		...
		switch ($operate) {
			case 'delete':
				$model=M('WeightUnit');
				$model->where(['weight_unit_id'=>['in',I('post.selected')]])->delete();
				break;
		}
