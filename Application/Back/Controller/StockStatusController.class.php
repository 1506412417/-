...
class StockStatusController extends Controller
{
	...
	public function editAction(){
		...
		if(IS_POST){
			if($model->create()){//校验
				...
				session('data', $_POST);
				$this->redirect('edit', ['stock_status_id'=>I('post.stock_status_id')]); // 重定向到添加
			} 
		}else{
			$this->assign('message',session('message'));
			session('message',null);
			// 获取当前编辑的内容, 如果是编辑错误,则显示错误的内容, 如果是没有错误, 则显示原始数据内容
			 $this->assign('data', is_null(session('data')) ? $model->find(I('get.stock_status_id')) : session('data'));
			 session('data',null);
	...
	public function multiAction(){
		...
		switch ($operate) {
			case 'delete':
				$model=M('StockStatus');
				$model->where(['stock_status_id'=>['in',I('post.selected')]])->delete();
				break;
		}
