    ...
    public function loginAction()
    {
        if (IS_POST) {
            ...
            if ($member = $model_member->checkMember(I('post.email', '', 'trim'), I('post.password','', 'trim'))) {
                // 会员合法
                // 二: session中的登录标志
                unset($member['password']);//销毁密码
                unset($member['password_salt']);//销毁盐值
                session('member', $member);

                // 三: 更新登录日志(AR操作)
                $model_mll = M('MemberLoginLog');
                $model_mll->member_id = $member['member_id'];
                $model_mll->login_time = time(); //登陆时间
                $model_mll->login_ip = get_client_ip(1);//获取客户端Ip
                $model_mll->login_ua = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '';//用户代理
                $model_mll->add();

                // 四: 同步购物车中的商品
                $cart = Cart::instance();//实例化购物车
                $cart->memberRefresh();//刷新购物车
