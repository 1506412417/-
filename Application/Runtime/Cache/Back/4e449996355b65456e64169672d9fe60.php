        ...
        <div class="container-fluid">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <!-- /entry.php/Back/Role/access -->
                    <form action="<?php echo U('');?>" method="post" enctype="multipart/form-data" id="form-brand" class="form-horizontal">
                        <input type="hidden" name="id" id="input-id" value="<?php echo ($id); ?>">
                        <ul class="nav nav-tabs">
                            <?php if(is_array($nodeNested)): $i = 0; $__LIST__ = $nodeNested;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$node): $mod = ($i % 2 );++$i;?><li class="<?php if($i == '1'): ?>active<?php endif; ?>">
                                <a href="#tab-<?php echo ($node['name']); ?>" data-toggle="tab"><?php echo ($node['title']); ?></a>
                                </li><?php endforeach; endif; else: echo "" ;endif; ?>

                        </ul>
                        <div class="tab-content">
                            <?php if(is_array($nodeNested)): $i = 0; $__LIST__ = $nodeNested;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$module): $mod = ($i % 2 );++$i;?><!--第1次遍历获取模块-->
                                <div class="tab-pane <?php if($i == '1'): ?>active<?php endif; ?>" id="tab-<?php echo ($module['name']); ?>">
                                <?php if(is_array($module['child'])): $i = 0; $__LIST__ = $module['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$controller): $mod = ($i % 2 );++$i;?><!--第2次遍历获取到控制器-->
                                    <div class="form-group required">

                                        <label class="col-sm-2 control-label" for="input-access-<?php echo ($controller['name']); ?>">
                                            <!--设置点击事件，找到所在div下所有复选框，中的checked属性 值设为和当前对象属性相同，如果设为true则模块无法取消选中-->
                                            <input id="input-access-<?php echo ($controller['name']); ?>" type="checkbox" onclick="$(this).parents('div.form-group').find(':checkbox').prop('checked', $(this).prop('checked'))">
                                            <?php echo ($controller['title']); ?></label>
                                        <div class="col-sm-10">
                                            <div class="row">
                                                <?php if(is_array($controller['child'])): $i = 0; $__LIST__ = $controller['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$action): $mod = ($i % 2 );++$i;?><!--第3次遍历获取动作-->
                                                    <div class="col-sm-2">
                                                        <input type="checkbox" name="access[]" value="<?php echo ($action['id']); ?>" id="input-access-<?php echo ($action['name']); ?>" class="form-control" <?php if(in_array($action['id'], $accessList)): ?>checked<?php endif; ?> />
                                                        <label for="input-access-<?php echo ($action['name']); ?>"><?php echo ($action['title']); ?></label>
