          ...
          <form action="" method="post" enctype="multipart/form-data" id="form-category">
            <div class="table-responsive">
              <table class="table table-bordered table-hover">
                ...
                <tbody>
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><!--判断等级是否大于0-->
                  <?php if($row['level'] > 0): ?><tr style="display: none;" data-level="<?php echo ($row['level']); ?>">
                      <?php else: ?>
                    <tr style="display: table-row;" data-level="<?php echo ($row['level']); ?>"><?php endif; ?>

                  <td class="text-center">
                    <input type="checkbox" name="selected[]" value="<?php echo ($row['category_id']); ?>" />
                  </td>
                  <td class="text-left">
                    <?php echo str_repeat('&nbsp;', $row['level']*4);?>
                  ...
                  <td class="text-right">
                    ...
                    <a href="<?php echo U('delete', ['category_id'=>$row['category_id']]);?>" data-toggle="tooltip" title="删除" class="btn btn-primary">
                      <i class="fa fa-trash-o"></i>
                    </a>
