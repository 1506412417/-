        ...
        <div class="container-fluid">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form action="<?php echo U('crud', ['step'=>$step]);?>" method="post" enctype="multipart/form-data" id="form-brand" class="form-horizontal">
                        <ul class="nav nav-tabs">
                            <li class="<?php if($step == 'table'): ?>active<?php endif; ?>">
                                <a href="#tab-table" data-toggle="tab">表信息</a>
                            </li>
                            <!-- condition条件 判断$step 是否等于 field -->
                            <li class="<?php if($step == 'field'): ?>active<?php endif; ?>">
                                <a href="#tab-field" data-toggle="tab">字段信息</a>
                            </li>
                        </ul>
                        <div class="tab-content">
                            <div class="tab-pane <?php if($step == 'table'): ?>active<?php endif; ?>" id="tab-table">

                                <div class="form-group required">
                                    <label class="col-sm-2 control-label" for="input-table">表名</label>
                                    <div class="col-sm-10">
                                        <input type="text" name="table" value="<?php echo ($data['table']); ?>" placeholder="无前缀表名" id="input-table" class="form-control" />
                                    </div>
                                </div>

                                <div class="form-group">
                                    <label class="col-sm-2 control-label" for="input-title">表展示标题</label>
                                    <div class="col-sm-10">
                                        <input type="text" name="title" value="<?php echo ($data['title']); ?>" placeholder="展示标题, 默认使用表名" id="input-title" class="form-control">
                                    </div>
                                </div>

                            </div>

                            <div class="tab-pane <?php if($step == 'field'): ?>active<?php endif; ?>" id="tab-field">

                                <div class="row">
                                    <div class="col-sm-12">
                                        <table id="table-field" class="table table-striped table-bordered table-hover">
                                            <thead>
                                            <tr>
                                                <td class="text-left">字段</td>
                                                <td class="text-left">展示标题</td>
                                                <td class="text-center">是否设置</td>
                                                <td class="text-center">是否列表</td>
                                                <td class="text-center">是否排序</td>
                                                <td class="text-center">是否搜索</td>
                                            </tr>
                                            </thead>
                                            <tbody>
                                            <?php if(is_array($fieldList)): $i = 0; $__LIST__ = $fieldList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$field): $mod = ($i % 2 );++$i;?><tr id="tr-field-title">
                                                <td class="text-left">
                                                    <?php echo ($field); ?>
                                                </td>
                                                <td class="text-left">
                                                    <input name="field[<?php echo ($field); ?>][title]" value="" placeholder="标题" class="form-control" type="text">
                                                </td>
                                                <td class="text-center">
                                                    <input type="checkbox" name="field[<?php echo ($field); ?>][is_set]" value="1" checked>
                                                </td>

                                                <td class="text-center">
                                                    <input type="checkbox" name="field[<?php echo ($field); ?>][is_list]" value="1" checked>
                                                </td>
                                                <td class="text-center">
                                                    <input type="checkbox" name="field[<?php echo ($field); ?>][is_sort]" value="1" checked>
                                                </td>
                                                <td class="text-center">
                                                    <input type="checkbox" name="field[<?php echo ($field); ?>][is_search]" value="1" checked disabled>
                                                </td>
